...
h5f_val = h5py.File('dataset_val.h5', 'r')
X = h5f_train['X']
Y = h5f_train['Y']
X_test = h5f_test['X']
X_val= h5f_val['X']
...
img_size = 227
label_size = 5
model_name = 'flowerClassify_load'
...
model.load('models/flowerClassify/ckpt-779')
model.fit({'input': X}, {'target': Y}, n_epoch=100,
           validation_set=({'input': X_val}, {'target': Y_val}),
           snapshot_step=100, show_metric=True, run_id='convnet_mnist')
model.save('models/mymodel_load.tflearn')
model.load('models/mymodel_load.tflearn')
predictions = model.predict_label(X_test)
new_y = np.argmax(predictions, axis=1)
np.savetxt('prediction_load.txt',new_y,fmt="%d",newline='\n')
